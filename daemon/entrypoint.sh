...
function osd_activate {
  ...
  if [[ ! -z "${OSD_JOURNAL}" ]]; then
    ...
    chown ceph. ${OSD_JOURNAL}
    ceph-disk -v  activate $(dev_part ${OSD_DEVICE} 1)
...
#!/bin/bash
echo "store-daemon: starting daemon on ${HOSTNAME}..."
exec /usr/bin/ceph-osd ${CEPH_OPTS} -f -d -i ${2} 
EOF
  chmod +x /etc/service/${1}-${2}/run
}

function enable_python3 {
...
#        chown ceph. ${OSD_JOURNAL}
        ceph-disk -v  activate $(dev_part ${OSD_DEV} 1)
      else
...
function osd_activate_journal {
  ...
  chown ceph. /var/lib/ceph/osd
  chown ceph. ${OSD_JOURNAL}
  ceph-disk -v  activate-journal ${OSD_JOURNAL}
